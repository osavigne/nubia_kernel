...
#include	<linux/i2c/lsm330dlc.h> /*ChenxiMao*/
...
static int lsm330dlc_acc_i2c_write(struct lsm330dlc_acc_status *stat, u8 *buf,
									int len)
{
	int ret;
	u8 reg, value;
...
static int lsm330dlc_acc_device_power_on(struct lsm330dlc_acc_status *stat)
{
	...
	dev_info(&stat->client->dev, "power on\n");
	return 0;
}

static irqreturn_t lsm330dlc_acc_isr1(int irq, void *dev)
{
	struct lsm330dlc_acc_status *stat = dev;
...
static void lsm330dlc_acc_irq1_work_func(struct work_struct *work)
{
	...
	container_of(work, struct lsm330dlc_acc_status, irq1_work);
	/* TODO  add interrupt service procedure.
		 ie:lsm330dlc_acc_get_int1_source(stat); */
...
static void lsm330dlc_acc_irq2_work_func(struct work_struct *work)
{
	...
	struct lsm330dlc_acc_status *stat =
	container_of(work, struct lsm330dlc_acc_status, irq2_work);
	/* TODO  add interrupt service procedure.
		 ie:lsm330dlc_acc_get_tap_source(stat); */
		...
		const struct i2c_device_id *id)
{
	...
	if (client->dev.platform_data == NULL) {
		...
		memcpy(stat->pdata, &default_lsm330dlc_acc_pdata,
							sizeof(*stat->pdata));
	...
	if (stat->pdata->gpio_int1 >= 0) {
		...
		pr_info("%s: %s has set irq1 to irq: %d, "
							"mapped on gpio:%d\n",
			LSM330DLC_ACC_DEV_NAME, __func__, stat->irq1,
	...
	if (stat->pdata->gpio_int2 >= 0) {
		...
		pr_info("%s: %s has set irq2 to irq: %d, "
							"mapped on gpio:%d\n",
			LSM330DLC_ACC_DEV_NAME, __func__, stat->irq2,
...
*/
	...
	if (stat->pdata->gpio_int1 >= 0) {
		INIT_WORK(&stat->irq1_work, lsm330dlc_acc_irq1_work_func);
		...
		err = request_irq(stat->irq1, lsm330dlc_acc_isr1,
			IRQF_TRIGGER_RISING, "lsm330dlc_acc_irq1", stat);
		if (err < 0) {
	...
	if (stat->pdata->gpio_int2 >= 0) {
		INIT_WORK(&stat->irq2_work, lsm330dlc_acc_irq2_work_func);
		...
		err = request_irq(stat->irq2, lsm330dlc_acc_isr2,
			IRQF_TRIGGER_RISING, "lsm330dlc_acc_irq2", stat);
		if (err < 0) {
